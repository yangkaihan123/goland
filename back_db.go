...
func main() {
	viper.SetConfigName("mysql-config")
	...
	sqlPath := viper.GetString("mysql.sqlPath")
	fmt.Println(host, port, user, password, databaseName, tableName, sqlPath)
	BackupMySqlDb(host, port, user, password, databaseName, tableName, sqlPath)
}
...
func BackupMySqlDb(host, port, user, password, databaseName, tableName, sqlPath string) (error, string) {
	...
	if tableName == "" {
		...
		cmd = exec.Command("mysqldump", "--opt", "-h "+host, "-P "+port, "-u "+user, "-p "+password, databaseName, "-t "+tableName)
		fmt.Println(cmd)
	}
	...
	//获得一个当前的时间戳
	now := time.Now().Format("20220623")
	var backupPath string

	//设置我们备份文件的名字
	if tableName == "" {
		backupPath = sqlPath + databaseName + now + ".sql"
	} else {
		backupPath = sqlPath + databaseName + tableName + "_" + now + ".sql"
	}
	//写入文件并设置文件权限
